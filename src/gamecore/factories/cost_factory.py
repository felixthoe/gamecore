...
def make_random_costs(
    system: LinearSystem,
    q_i_def: str = "pd",
    r_i_jj: str = "free",             # Diagonal terms u[j].T R_i,jj u[j]
    r_i_jk: str = "zero",             # Cross terms j≠k u[j].T R_i,jk u[k]
    ...
    r_i_jk : str
        ...
        "free" requires r_i_jj in {"free", "psd"} to be valid.
        Positive semidefiniteness of the overall block matrix R_i is enforced if enforce_psd_r_i is True.
    ...
    if r_i_jj == "zero":
        ...
        raise ValueError(f"Cost Factory: Unknown value for r_i_jj '{r_i_jj}'. Use 'zero', 'psd', or 'free'.")
    ...
    if r_i_jk == "zero":
        ...
        if r_i_jj == "zero" and enforce_psd_r_i:
            raise ValueError("Cost Factory: r_i_jk='free' with r_i_jj='zero' and enforce_psd_r_i=True is not valid. The overall block matrix R_i will in general not be positive semidefinite and cannot be adjusted to be so without modifying the zero R_i_jj blocks. Consider setting r_i_jj to 'psd' or 'free', or setting enforce_psd_r_i to False.")
        ...
        raise ValueError(f"Cost Factory: Unknown value for r_i_jk '{r_i_jk}'. Use 'zero' or 'free'.")
    ...
    for i in range(N):
        ...
        if min_eig < 0:
            if not enforce_psd_r_i:
                ...
                if r_i_jj == "zero":
                    raise ValueError(
                        "Cost Factory: Cannot enforce positive semidefiniteness of R_i with r_i_jj='zero' when cross terms are present. We should conceptionally not reach this point as an Exception should have been raised to catch this earlier."
                    )
